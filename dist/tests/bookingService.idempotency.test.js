...
describe('BookingService idempotency', () => {
    const service = new bookingService_1.BookingService();
    beforeEach(() => {
        jest.clearAllMocks();
    });
    it('returns existing booking when create fails with P2002', async () => {
        ...
        prisma.user.findUnique.mockResolvedValue({ id: 'creator-1', name: 'Creator' });
        ...
        const result = await service.createBooking(bookingData, 'creator-1', idempotencyKey);
